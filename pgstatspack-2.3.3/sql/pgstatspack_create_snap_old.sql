...
-- meo.bogliolo@xenialab.it 9.6 support for activity table
 ...
 BEGIN
  ...
  if found then
    begin
      ...
      insert into pgstatspack_names (name)
      select distinct pg_get_userbyid(userid)
      from pg_stat_statements
...
insert into pgstatspack_names (name)
 ...
 left join pgstatspack_names on usename=name
 where
...
insert into pgstatspack_names (name)
 ...
 from pg_stat_activity
 left join pgstatspack_names on application_name=name
 where
  datid=(select oid from pg_database where datname=current_database()) and
  name is null;
...
insert into pgstatspack_names (name)
 select '<IDLE>'
 where not exists(select 1 from pgstatspack_names where name='<IDLE>');

IF ((version_major = 9 AND version_minor >= 6 ) OR version_major > 9 ) THEN
BEGIN
insert into pgstatspack_names (name)
 select distinct (case state when 'idle' then '<IDLE>' else query end)
 from pg_stat_activity
 left join pgstatspack_names on query=name
 where
  datid=(select oid from pg_database where datname=current_database()) and
  name is null and state<>'idle';
INSERT INTO pgstatspack_activity
    (snapid, procpid, user_name_id, application_name_id, client_addr, waiting, query_id)
    SELECT
      spid,
      a.pid,
      n1.nameid,
      n2.nameid,
      a.client_addr,
      a.wait_event is null,
      n3.nameid
    FROM pg_stat_activity a
    join pgstatspack_names n1 on a.usename=n1.name
    join pgstatspack_names n2 on a.application_name=n2.name
    join pgstatspack_names n3 on a.query=n3.name
    WHERE datid=(select oid from pg_database where datname=current_database()) and a.state<>'idle';
INSERT INTO pgstatspack_activity
    (snapid, procpid, user_name_id, application_name_id, client_addr, waiting, query_id)
    SELECT
      spid,
    ...
    join pgstatspack_names n1 on a.usename=n1.name
    join pgstatspack_names n2 on a.application_name=n2.name
    join pgstatspack_names n3 on n3.name='<IDLE>'
    WHERE datid=(select oid from pg_database where datname=current_database()) and a.state='idle';
END;
ELSE
...
ELSE
insert into pgstatspack_names (name)
 select distinct current_query
 from pg_stat_activity
 left join pgstatspack_names on current_query=name
 where
  datid=(select oid from pg_database where datname=current_database()) and
  name is null;
INSERT INTO pgstatspack_activity
    ...
    join pgstatspack_names n1 on a.usename=n1.name
    join pgstatspack_names n2 on a.application_name=n2.name
    join pgstatspack_names n3 on a.current_query=n3.name
