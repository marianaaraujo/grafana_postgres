 ...
 BEGIN
  ...
  if found then
    begin
      ...
      insert into pgstatspack_names (name)
      select distinct substring(query from 1 for 512)
      from pg_stat_statements
      left join pgstatspack_names on substring(query from 1 for 512)=name
      where
    ...
    join pgstatspack_names n1 on substring(s.query from 1 for 512) =n1.name
    join pgstatspack_names n2 on pg_get_userbyid(s.userid)=n2.name
    WHERE dbid=(select oid from pg_database where datname=current_database())
    GROUP BY spid, n1.nameid, n2.nameid
    ORDER BY total_time;
...
ELSE
INSERT INTO pgstatspack_activity
    (snapid, count_start, user_name_id, application_name_id, client_addr, waiting, working)
    SELECT
      spid,
      a.procpid,
      n1.nameid,
      ...
      coalesce(a.client_addr,'0.0.0.0/0'),
      a.waiting,
      case when a.query='<idle>' then false else true end
    FROM pg_stat_activity a
    left join pgstatspack_names n1 on coalesce(a.usename, 'NONE')=n1.name
    join pgstatspack_names n2 on coalesce(a.application_name, 'NONE')=n2.name
    WHERE datid=(select oid from pg_database where datname=current_database())
    GROUP BY spid, n1.nameid, n2.nameid, coalesce(a.client_addr,'0.0.0.0/0'), a.waiting, case when a.query='<idle>' then false else true end;
 END IF;
END IF;
