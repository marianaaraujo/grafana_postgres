...
DROP TABLE if exists pgstatspack_statements;
...
DROP TABLE if exists pgstatspack_functions;
...
DROP TABLE if exists pgstatspack_bgwriter;
...
DROP TABLE if exists pgstatspack_space_usage;
CREATE TABLE pgstatspack_space_usage
(
  ts timestamp without time zone NOT NULL, 
  schema name NOT NULL,
  sizek bigint, 
  CONSTRAINT pgstatspack_space_usage_pk PRIMARY KEY (ts , schema )
);
...
SELECT snapid, count_star, n1.name as user_name, n2.name as application_name, client_addr, waiting, working
