...
# enhanced by meo bogliolo
#	added stats for pg_stat_activity
#	fixed 2 queries
...
printf "\nactivity stats\n" | tee -a $FILENAME
$PSQL --user $PGUSER --dbname $PGDB --quiet --command "
SELECT a.tot as current_sessions, a.active as current_active_sessions, 
       b.tot as old_sessions, b.active as old_active_sessions,
       a.tot-b.tot as delta_sessions, a.active-b.active as delta_active_sessions
FROM   (select count(*) as tot, sum(CASE current_query WHEN '<IDLE>' THEN 0 ELSE 1 END) as active 
        from pgstatspack_activity_v WHERE
        snapid=$STARTSNAP) a,
       (select count(*) as tot, sum(CASE current_query WHEN '<IDLE>' THEN 0 ELSE 1 END) as active 
        from pgstatspack_activity_v WHERE
        snapid=$STOPSNAP) b
" | tee -a $FILENAME
